...
coverImage: "/images/site/my-clean-blog1.webp"
tools:
  - "Proxmox VE"
  - "Atomic Red Team"
  - "LimaCharlie"
...
2. Deploy the following VMs from templates:
   - `win11-target` — used for execution and user simulation.
   - `win2019-fileserver` — houses sample data sets.
   - `ubuntu-c2` — orchestrates Atomic Red Team and hosts the command-and-control emulator.
3. Enable daily snapshots so the environment can be rolled back quickly after encryption runs.

## ⚙️ Sensor Instrumentation
...
2. Execute the following atoms sequentially:
   - `T1486` — encrypt files via `7zip` payload.
   - `T1059.001` — PowerShell execution of the ransom note dropper.
